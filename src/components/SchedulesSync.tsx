...
import { 
  ...
  doc, writeBatch, query, getCountFromServer, limit as firestoreLimit,
...
import { FALLBACK_HAND_AUDITS, FALLBACK_PATIENT_AUDITS, FALLBACK_SURGERY_AUDITS } from '../lib/fallbackData';
...
export default function SchedulesSync() {
  ...
  const fetchCounts = async () => {
    if (localStorage.getItem('firestore_quota_exceeded') === 'true') {
      try {
        ...
        setCounts({
          tracer_01: FALLBACK_PATIENT_AUDITS.length,
          tracer_02: FALLBACK_SURGERY_AUDITS.length,
          tracer_03: FALLBACK_HAND_AUDITS.length
        });
      }
      return;
    }
    ...
    try {
      ...
      console.error("Error fetching counts, loading fallback catalog counts:", e);
  ...
  useEffect(() => {
    ...
    // Sincronização automática e em tempo real das contagens na tela
    const unsubs = TRACER_CONFIGS.map(config => {
      const q = query(collection(db, config.collection), firestoreLimit(1));
      return onSnapshot(q, () => {
        fetchCounts();
      }, (err) => {
        console.error("SchedulesSync count onSnapshot failed:", err);
  ...
  const handleClear = async (tracerId: string) => {
    ...
    try {
      ...
      // Re-fetch all counts to be sure
  ...
  const handleSync = async (tracerId: string) => {
    ...
    try {
      ...
      addLog(`Erro (${tracerId}): ${err.message}`, 'error');
    } finally {
      setSyncingId(null);
      fetchCounts();
    }
