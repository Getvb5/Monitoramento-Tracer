...
export default function Dashboard({ 
  ...
  useEffect(() => {
    ...
    unsubHand = onSnapshot(qHand, (s) => {
      ...
      console.error("Dashboard hand audits loading failed:", err);
    ...
    unsubPatient = onSnapshot(qPatient, (s) => {
      ...
      console.error("Dashboard patient ID loading failed:", err);
      if (err?.message && (err.message.includes('Quota') || err.message.includes('resource-exhausted') || err.message.includes('quota') || err.message.includes('limit'))) {
        handleQuotaFailure();
      } else {
    ...
    unsubSurgery = onSnapshot(qSurgery, (s) => {
      ...
      console.error("Dashboard surgery loading failed:", err);
      if (err?.message && (err.message.includes('Quota') || err.message.includes('resource-exhausted') || err.message.includes('quota') || err.message.includes('limit'))) {
        handleQuotaFailure();
      } else {
    ...
    // Escutar por cota excedida de outros componentes
    ...
    const timer = setTimeout(() => {
      setLoading(false);
      // Se ainda nao carregou, use os fallbacks mesclados
  ...
  const complianceMetrics = useMemo(() => {
    ...
    filteredPatient.forEach(a => {
      if (a.hasWristband === true || String(a.hasWristband).toUpperCase().trim() === 'SIM') t01HasWristband++;
      if (a.wristbandLegible === true || String(a.wristbandLegible).toUpperCase().trim() === 'SIM') t01WristbandLegible++;
      if (a.correctData === true || String(a.correctData).toUpperCase().trim() === 'SIM') t01CorrectData++;
    });
    ...
    filteredSurgery.forEach(a => {
      if (a.signIIn === true || String(a.signIIn).toUpperCase().trim() === 'SIM') t02SignIn++;
      if (a.timeOut === true || String(a.timeOut).toUpperCase().trim() === 'SIM') t02TimeOut++;
      if (a.signOut === true || String(a.signOut).toUpperCase().trim() === 'SIM') t02SignOut++;
    });
    ...
    filteredHand.forEach(a => {
      if (a.compliant === true || String(a.compliant).toUpperCase().trim() === 'SIM') t03Compliant++;
    });
  ...
  const itemsComplianceData = useMemo(() => {
    ...
    return [
      ...
      { name: 'Higiene das Mãos', pct: t03Total > 0 ? Math.min(100, Math.round((t03Compliant / t03Total) * 100)) : 0, color: '#6366f1', tracer: 'T03' },
    ];
  }, [complianceMetrics]);
  ...
  const unitsComplianceData = useMemo(() => {
    ...
    filteredPatient.forEach(a => {
      ...
      let con = 0;
      if (a.hasWristband === true || String(a.hasWristband).toUpperCase().trim() === 'SIM') con++;
      if (a.wristbandLegible === true || String(a.wristbandLegible).toUpperCase().trim() === 'SIM') con++;
      if (a.correctData === true || String(a.correctData).toUpperCase().trim() === 'SIM') con++;
    ...
    filteredSurgery.forEach(a => {
      const uid = a.unitId || a.hospitalId || a.unidadeId;
      let con = 0;
      if (a.signIIn === true || String(a.signIIn).toUpperCase().trim() === 'SIM') con++;
      if (a.timeOut === true || String(a.timeOut).toUpperCase().trim() === 'SIM') con++;
      if (a.signOut === true || String(a.signOut).toUpperCase().trim() === 'SIM') con++;
      addAuditToUnit(uid, con, 3);
    });

    filteredHand.forEach(a => {
      const uid = a.unitId || a.hospitalId || a.unidadeId;
      let con = (a.compliant === true || String(a.compliant).toUpperCase().trim() === 'SIM') ? 1 : 0;
  ...
  const tracerShareData = useMemo(() => {
    return [
      ...
      { name: 'T02 - Safe Surgery', value: filteredSurgery.length, color: '#f59e0b' },
      { name: 'T03 - Higiene Mãos', value: filteredHand.length, color: '#6366f1' }
    ].filter(t => t.value > 0);
  }, [filteredPatient, filteredSurgery, filteredHand]);
  ...
  const strategicInsightsList = useMemo(() => {
    ...
    if (t03Total > 0) {
      const hygienePct = Math.round((t03Compliant / t03Total) * 100);
      if (hygienePct < 80) {
        insights.push({
          type: 'danger',
          title: 'Conformidade Crítica em Higiene das Mãos',
          desc: `O índice de higiene das mãos é de apenas ${hygienePct}%. Este é o principal vetor de infecção hospitalar. Ação educativa conjunta urgente recomendada.`,
          tracer: 'T03'
        });
      } else if (hygienePct < 90) {
        insights.push({
          type: 'warning',
          title: 'Meta de Higiene das Mãos Próxima',
          desc: `A conformidade de higiene prática em procedimentos está em ${hygienePct}%. Ampliar displays de álcool gel para bater a meta de 90%.`,
          tracer: 'T03'
        });
      } else {
        insights.push({
          type: 'success',
          title: 'Higiene das Mãos Nível OMS',
          desc: `Higienização prática de procedimentos mantida de forma exemplar em ${hygienePct}%, mitigando infecções cruzadas operacionais.`,
          tracer: 'T03'
        });
      }
    }
  ...
  const primaryStats = [
    ...
    { label: 'T02 - Proc. Cirúrgicos', value: `${filteredSurgery.length}`, icon: <Scissors className="w-5 h-5 text-amber-600" />, color: 'text-amber-600', bg: 'bg-amber-50/50', border: 'border-amber-100', glow: 'shadow-amber-50/10' },
    { label: 'T03 - Proc. Medicação', value: `${filteredHand.length}`, icon: <Pill className="w-5 h-5 text-indigo-600" />, color: 'text-indigo-600', bg: 'bg-indigo-50/50', border: 'border-indigo-100', glow: 'shadow-indigo-50/10' },
    { label: 'Respostas Totais', value: filteredHand.length + filteredPatient.length + filteredSurgery.length, icon: <TrendingUp className="w-5 h-5 text-slate-800" />, color: 'text-slate-800', bg: 'bg-slate-50/50', border: 'border-slate-100', glow: 'shadow-slate-50/10' },
  ];
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <div className="flex items-baseline gap-2 mt-1">
                <p className="text-3xl font-black text-slate-900 tracking-tight leading-none group-hover:scale-[1.01] transition-transform duration-250">
                  {s.value}
                </p>
                {s.label.includes('T0') && (
                  <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-wider shrink-0">auditorias</span>
                )}
              </div>
            </div>
            
            <div className="relative z-10 mt-5 pt-3 border-t border-slate-100 flex items-center justify-between text-[8px] font-black uppercase text-slate-400 tracking-wider">
              <span>Monitorado On-line</span>
